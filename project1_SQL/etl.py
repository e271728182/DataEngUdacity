...
def process_log_file(cur, filepath):
    ...
    time_data=df.filter(['ts'], axis=1)
#time_data['ts']=pd.to_datetime(df.ts,unit='ms')

    time_data['datetime']=pd.to_datetime(time_data.ts,unit='ms')
    time_data['hour']=time_data['datetime'].dt.hour
    time_data['day']=time_data['datetime'].dt.day
    time_data['weekOfYear']=time_data['datetime'].dt.week
    ...
    time_data=time_data.drop(columns=['datetime'])
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = songplay_data = (index, pd.to_datetime(row.ts, unit='ms'), int(row.userId), row.level, songid, artistid, row.sessionId, row.location, row.userAgent)
